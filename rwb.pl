...
if ($action eq "base") {
  ...
  eval {
      @opinionperm = ExecSQL($dbuser, $dbpasswd,
                             "select name
                             from rwb_permissions
                             where action='give-opinion-data'"
      );
  };
  foreach (@opinionperm) {
      if ("@{ $_ }" eq $user) {
  ...
  print "<form>";
  print
      "<input type=\"checkbox\" name=\"summary\" id=\"summary\" values=1 onclick=ViewShift()>Summary<br>";
  print "</form>";

 #  # checkbox for state.
 #    print "<form>";
 #    print
 #        "<input type=\"checkbox\" name=\"state\" id=\"state\" values=1 onclick=ViewShift()>State<br>";
 #    print "</form>";
 # #
 #
  ...
  if ($sum eq "summary") {
    if ($what{committees}) {
        my ( $cm2cmt, $c2cm_color, $error1 )
            = Aggr_Comm2Comm( $latne, $longne, $latsw, $longsw, $cycleSQL,$format);
        if ($error1) {
        ...
        my ( $cm2cnd, $c2cd_color, $error2 )
            = Aggr_Comm2Cand( $latne, $longne, $latsw, $longsw, $cycleSQL,$format);
        if ($error2) {
     ...
     if ($what{opinions} ) {
      # if(UserCan($user,"query-opinion-data")){
        my ( $opn, $op_color, $error4 )
            = Aggr_Opinions( $latne, $longne, $latsw, $longsw, $cycleSQL, $format);
        if ($error4) {
      ...
      # }else{
      #   print "You are not permiited to look up opinions!";
      # }
     }
    }
  } else {                                          #if summary isn't choosen.
    if ($what{committees}) {
...
if ($action eq "invite-user") {
  ...
  if (UserCan($user,"invite-users")){
    if (!$run) {
      ...
      open TXT,">email.txt";
      $rlink = rand(999999);
      print TXT "Hi, you are invited to join RWB system\n";

      print TXT "Administration Link:\n";
      print TXT "http://murphy.wot.eecs.northwestern.edu/~mcm9021/rwb/rwb.pl?keys=$rlink&act=nInvite\n";
      close TXT;
      ...
      my ($emailstatus) = `cat ~/www/rwb/email.txt | mail -s "This is a test!" $email`;

      if($emailstatus){
        print "Email is sent succesfully!"
      }else{
        print "ERROR: Email is not sent succesfully!"
      }
    }

  }else{
    print "You don't have enough permission, please contact with root"
  }
  print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
}

if($action eq "nInvite"){
  print h2("Please enter your information!\n");
  # read the rilin value
  open(my $FL, '<', 'random.txt');
  $rlink = <$FL> ;
  if (!$run) {
    if(($key - $rlink) == 0){
      open TXT, ">random.txt";
      ...
      "Refer ", textfield(-name=>'refer'),p,
      hidden(-name=>'run',-default=>['1']),p,
      hidden(-name=>'act',-default=>['user-add']),
      submit, end_form, hr;
    }
  }else{
    # ADD USER PART
    ...
    my $error = UserAdd($name,$password,$email,$refer);
    # ADD PERMISSION
    if($error){
      print h2("Fail! Please contact with root!");
    }

    my @COL;
    eval { @COL = ExecSQL($dbuser,$dbpasswd,
     "select rwb_permissions.action from rwb_permissions where rwb_permissions.name = '$refer' ", undef,@_);};

    for my $tuple (@COL) {
      my $error_perm = GiveUserPerm($name,${$tuple}[0]);
    }
  }
  print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
}
...
if ($action eq "give-opinion-data") {
  if(!UserCan($user,"give-opinion-data")){
    ...
    if($run == 0){
      print start_form(-name=>'AddOpinion'),
      h2('beimg implemented by chen'),
      "Opinion(between -1 and 1) ", textfield(-name=>'opinion'),p,
      "Submitter", textfield(-name=>'submitter'),p,
      hidden(-name=>'location', -id=>'location'),
      hidden(-name=>'run',-default=>['1']),p
      ...
      my @temp = split(',', param('location'));
      my $lat = $temp[0];
      my $long = $temp[1];
      my $opinion = param("opinion");
      my $submitter = param("submitter");
      my $error = OpinionAdd($submitter,$opinion,$lat,$long);
    }
  }
  print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
}
...
sub Committees {
  ...
  eval {
    @rows = ExecSQL($dbuser,
                    ...
                    "select latitude, longitude, cmte_nm, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip
                     from cs339.committee_master natural join cs339.cmte_id_to_geo
                     where latitude>? and latitude<? and longitude>? and longitude<? $cycleSQL",
                     undef,
                     $latsw,
                     $latne,
                     $longsw,
...
sub Candidates {
  ...
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd,
                    ...
                    "select latitude, longitude, cand_name, cand_pty_affiliation, cand_st1, cand_st2, cand_city, cand_st, cand_zip
        ...
        eval {
            @dems = ExecSQL(
                ...
                "select sum(transaction_amnt)
                 from (select cmte_ID, cmte_pty_affiliation, cycle
                       from cs339.committee_master
                       where cmte_pty_affiliation in ('DEM','Dem','dem'))
                    natural join
                       cs339.cmte_id_to_geo
                    natural join
                       (select cmte_id, transaction_amnt from cs339.comm_to_cand)
                 where latitude>? and latitude<? and longitude>? and longitude<? $cycleSQL",
                undef,
                $latsw,
                $latne,
        ...
        eval {
            @reps = ExecSQL(
                ...
                "select sum(transaction_amnt)
                 from (select cmte_ID, cmte_pty_affiliation, cycle
                       from cs339.committee_master
                       where cmte_pty_affiliation in ('REP','Rep','rep'))
                    natural join
                      cs339.cmte_id_to_geo
                    natural join
                      (select cmte_id, transaction_amnt from cs339.comm_to_cand)
                ...
                "select sum(transaction_amnt)
                 from (select cmte_ID, cmte_pty_affiliation, cycle
                    ...
                    natural join
                      cs339.cmte_id_to_geo
                    natural join
                      (select cmte_id, transaction_amnt from cs339.comm_to_comm)
                 where latitude>? and latitude<? and longitude>? and longitude<? $cycleSQL",
                undef,
                $latsw,
                $latne,
                ...
                "select sum(transaction_amnt)
                 from (select cmte_ID, cmte_pty_affiliation, cycle
                    ...
                    natural join
                       cs339.cmte_id_to_geo
                    natural join
                       (select cmte_id, transaction_amnt
...
sub Individuals {
  ...
  eval {
    @rows = ExecSQL($dbuser, $dbpasswd,
                    ...
                    "select latitude, longitude, name, city, state, zip_code, employer, transaction_amnt
    ...
    while ( ( $dem == 0 || $rep == 0 ) && $try <= 2 ) {
        ...
        eval {
            @dems = ExecSQL(
                ...
                "select sum(transaction_amnt)
                 from (select cmte_ID, cmte_pty_affiliation
                       from cs339.committee_master
                       where cmte_pty_affiliation in ('DEM','Dem','dem'))
                    natural join
                       cs339.ind_to_geo
                    natural join
                       (select cmte_id, transaction_amnt, cycle, sub_id from cs339.individual)
                 where latitude>? and latitude<? and longitude>? and longitude<? $cycleSQL",
                undef,
                $latsw,
                $latne,
        ...
        eval {
            @reps = ExecSQL(
                ...
                "select sum(transaction_amnt)
                 from (select cmte_ID, cmte_pty_affiliation
                       from cs339.committee_master
                       where cmte_pty_affiliation in ('REP','rep','Rep','GOP'))
                    natural join
                       cs339.ind_to_geo
                    natural join
                       (select cmte_id, transaction_amnt, cycle, sub_id from cs339.individual)
...
# sub States {
#   my ($cycleSQL,$format) = @_;
#   my @dems;
#   eval {
#     @dems = ExecSQL($dbuser, $dbpasswd,
#                     "select CMTE_ST, sum(transaction_amnt)
#                     from (select CMTE_ST, cmte_pty_affiliation
#                           from cs339.committee_master
#                           where cmte_pty_affiliation in ('DEM','Dem','dem') $cycleSQL)
#                        natural join
#                           cs339.COMM_TO_COMM
#                     group by CMTE_ST",
#                      undef
...
#   } else {
#     if ($format eq "table") {
#       return (MakeTable("States_data", "2D",
# 			["state", "transaction_amnt"],
# 			@dems),$@);
#     } else {
#       return (MakeRaw("States_data","2D",@dems),$@);
#     }
#   }
# }
...
# Get the list of  a user's permissions
#
#
#
# GetUserPerm($name)
...
sub GetUserPerm {
  eval { ExecSQL($dbuser,$dbpasswd,
     "select rwb_permissions.action from rwb_permissions where rwb_permissions.name = ? ", undef,@_);};
  return $@;
}
